...
REQUEST_CONFIGS = YAML.load_file( File.open( ARGV.pop ) )

# Deal with defaults for historic attributes. Users can add to these with config, but created & modified will always exist.
BASE_HISTORIC_ATTRIBUTES = %w( created modified )
REQUEST_CONFIGS.each_value do |config|

  unless( config.include?( :historic_attributes ) )
    config[:historic_attributes] = Array.new
  end

  BASE_HISTORIC_ATTRIBUTES.each { |a| config[:historic_attributes] << a }

  # Finally, ensure we don't have duplicate field names.
...
def get_item_list( table_name, required_attributes, optional_attributes, historic_attributes, where_clause = '', clause_params = [] )
  result = Array.new
...
def get_required_attributes( table_name, id, required_attributes, optional_attributes )
  result = Hash.new
...
# Used by get_attributes. Please do not call this directly.
def get_metadata_attributes( table_name, id )
  result = Hash.new
...
def get_historic_attributes ( table_name, id, historic_attributes )
  result = Hash.new
...
def get_attributes( table_name, id, required_attributes, optional_attributes, historic_attributes )
  ensure_record_exists( table_name, id )

  result = Hash.new
  result.merge! get_required_attributes( table_name, id, required_attributes, optional_attributes )
  result.merge! get_metadata_attributes( table_name, id )
  result.merge! get_historic_attributes( table_name, id, historic_attributes )
  result
end
...
def update_attributes( table_name, id, input_data, required_attributes, optional_attributes )
...
def delete_meta_attributes( table_name, id, input_data )
...
def insert_attributes( table_name, input_data, required_attributes, optional_attributes )
...
get '/:type' do |type|
  with_protection do |input_data|
    ...
    get_item_list( cfg[:table_name], cfg[:required_attributes], cfg[:optional_attributes], cfg[:historic_attributes] )
  end
end
...
get '/:type/:id' do |type, id|
  with_protection do |input_data|
    ...
    get_attributes( cfg[:table_name], id, cfg[:required_attributes], cfg[:optional_attributes], cfg[:historic_attributes] )
  end
end
...
put '/:type/:id' do |type, id|
  with_protection do |input_data|
    ...
    update_attributes( cfg[:table_name], id, input_data, cfg[:required_attributes], cfg[:optional_attributes])
  end
end
...
post '/:type/:id/remove_attributes' do |type, id|
  with_protection do |input_data|
    ...
    delete_meta_attributes( cfg[:table_name], id, input_data )
  end
end
...
post '/:type/create' do |type|
  with_protection do |input_data|
    ...
    insert_attributes( cfg[:table_name], input_data, cfg[:required_attributes], cfg[:optional_attributes] )
  end
end
...
get '/:type/:id/:subtype' do |type, id, subtype|
  with_protection do |input_data|
    ...
    get_item_list( sub_cfg[:table_name], sub_cfg[:required_attributes], sub_cfg[:optional_attributes], sub_cfg[:historic_attributes], "WHERE #{ cfg[:table_name] }_id=?", [ id ] )
  end
end
